...
public class FreeConsultDetailsListPresenter extends BasePresenter<FreeConsultDetailsListContract.Model, FreeConsultDetailsListContract.View> {
    ...
    public void onCreate(int consultationId, int lawyerId, SmartRefreshLayout smartRefreshLayout) {
        ...
        adapter.setOnItemChildClickListener((adapter1, view, position) -> {
            ...
            switch (view.getId()) {
                case R.id.item_tv_delete:
                    new DeleteDialog(mApplication
                            , dialog -> deleteReply(entity.getConsultationReplyId(), position, dialog)
                            , mApplication.getString(R.string.text_sure_to_delete))
                            .show();
                    break;
    ...
    public void lawyerReply(String content) {
        ...
        if (content.length() < 20) {
            mRootView.showMessage(mApplication.getString(R.string.text_no_less_20));
            return;
        }
    ...
    private void deleteReply(int consultationReplyId, int position, Dialog dialog) {
