...
public class WorkExperienceAdapter extends BaseQuickAdapter<WorkexpEntity, BaseViewHolder> {

    private int type;//是否显示圆点和竖线

    public WorkExperienceAdapter(int type) {
    ...
    protected void convert(BaseViewHolder helper, WorkexpEntity item) {
        ...
        helper.setText(R.id.item_tv_name, item.getInstitutionName());
        helper.setText(R.id.item_tv_identity, item.getPositionName());
        switch (type) {
            case 1:
